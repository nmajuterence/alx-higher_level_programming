...
class Rectangle:
  ...
  methods for calculating area and perimeter, string
  representations,
  a custom deletion message, class attributes for
  tracking instances and
  specifying a print symbol.
  """

  number_of_instances = 0  # Public class attribute for instance count
  ...
  def __str__(self):
    ...
    for _ in range(self._height):
      row = self.print_symbol * self._width
      rows.append(row)
    return "\n".join(rows)
  ...
  def __del__(self):
    """
    Prints a custom message when a Rectangle object
    is deleted and decrements
    the number_of_instances counter.
    """
    print("Bye rectangle...")
